        ...
        with tf.compat.v1.variable_scope(name, initializer=initializer) as vs:
            ...
            if dropout:
                if type(dropout) in [tuple, list]:
                    in_rate = 1 -dropout[0]
                    out_rate = 1 -dropout[1]
                elif isinstance(dropout, float):
                    in_keep_prob, out_rate = 1 -dropout, dropout
                else:
                ...
                cell_creator = lambda: DropoutWrapper_fn(rnn_creator(),
                                                         input_rate=1-in_keep_prob,
                                                         output_rate=1-1.0)  # out_keep_prob)
            else:
...
class DynamicRNNLayer(Layer):
    ...
    def __init__(
        ...
        if dropout:
            ...
            #                     cell_instance_fn1(),
            #                     input_rate=1-in_keep_prob,
            #                     output_rate=1-out_keep_prob)
            cell_creator = lambda: DropoutWrapper_fn(rnn_creator(),
                      input_rate=1-in_keep_prob, output_rate=1-1.0)#out_keep_prob)
        else:
            cell_creator = rnn_creator
        ...
        if dropout:
            self.cell = DropoutWrapper_fn(self.cell,
                      input_rate=1-1.0, output_rate=1-out_keep_prob)
